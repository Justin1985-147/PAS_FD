function hpol = polarlq(varargin)
%由polar函数修改得到，不同的是以N为起始，顺时针计算角度.
%   POLARLQ(THETA, RHO) makes a plot using polar coordinates of
%   the angle THETA, in radians, versus the radius RHO.
%   POLARLQ(THETA,RHO,S) uses the linestyle specified in string S.
%   See PLOT for a description of legal linestyles.
%
%   POLARLQ(AX,...) plots into AX instead of GCA.
%
%   H = POLARLQ(...) returns a handle to the plotted object in H.
%
%   Example:
%      t = 0:.01:2*pi;
%      polarlq(t,sin(2*t).*cos(2*t),'--r')
%
%   修改者：刘琦
%   2011/04/15 11:00:00 $

%网格设置
nn=6;%180度内的角度个数，比如nn=6,则是30度的间隔。
...
% annotate spokes in degrees
    rt = 1.1*rmax;
    for i = 1:length(th)
        text(rt*snt(i),rt*cst(i),int2str(i*180/nn),...
             'horizontalalignment','center',...
             'handlevisibility','off','parent',cax);
        if i == length(th)
            loc = int2str(0);
        else
            loc = int2str(180+i*180/nn);
        end
        text(-rt*snt(i),-rt*cst(i),loc,'horizontalalignment','center',...
             'handlevisibility','off','parent',cax)
    end
    text(0,rt,['N';' ';' ';' '],'horizontalalignment','center',...
        'handlevisibility','off','parent',cax);
    text(rt,0,[' ',' ',' ',' ','E'],'horizontalalignment','left',...
        'handlevisibility','off','parent',cax);
    text(0,-rt,[' ';' ';' ';'S'],'horizontalalignment','center',...
        'handlevisibility','off','parent',cax);
    text(-rt,0,['W',' ',' ',' ',' '],'horizontalalignment','right',...
        'handlevisibility','off','parent',cax);
